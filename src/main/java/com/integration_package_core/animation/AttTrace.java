...
import com.integration_package_core.tool.RenderTypes;
...
import static net.minecraft.client.renderer.texture.OverlayTexture.RED_OVERLAY_V;
...
public class AttTrace {
    ...
    public static void render(MultiBufferSource bufferSource, PoseStack poseStack, PlayerAnimationEntity entity, float r1, float r2) {
        if (entity == null || entity.traceList.size() < 2) {
            return; // 轨迹点不足时直接返回
        }

        // 1. 选择半透明渲染类型（适合轨迹效果，可替换为你的发光类型）
        ResourceLocation trailTexture = new ResourceLocation(IntegrationPackageCore.MODID, "textures/animation_entity/trail.png");
        VertexConsumer vertexConsumer = bufferSource.getBuffer(RenderType.entityTranslucent(trailTexture));

        // 2. 遍历轨迹点，绘制连续四边形
        for (int i = 1; i < entity.traceList.size(); i++) {
            Matrix4f currMatrix = entity.traceList.get(i);       // 当前帧矩阵
            Matrix4f prevMatrix = entity.traceList.get(i - 1);   // 上一帧矩阵
            ...
            // 5. 计算纹理坐标（让纹理沿轨迹连续分布）
            float totalSegments = entity.traceList.size() - 1; // 总段数
