...
import com.integration_package_core.tool.collide.Line;
...
public class Render {
    ...
    public static float toTickFPS(){

        return (float) 20 /Minecraft.getInstance().getFps();
    ...
    public ResourceLocation texture = new ResourceLocation(IPC.MODID, "textures/air.png");
    public float u1 = 0;
    public float u2 = 512;
    public float v1 = 0;
    public float v2 = 512;
    ...
    public static Render of(PoseStack poseStack, RenderLevelStageEvent.Stage stage) {

        Render r = new Render(Minecraft.getInstance(), poseStack, Minecraft.getInstance().renderBuffers().bufferSource());
        r.stage = stage;
    ...
    public static Render ofLevel(PoseStack poseStack,Vector3f v) {

        Render.levelPoseStack(poseStack,v);
    ...
    public static void levelPoseStack(PoseStack poseStack, Vec3 v,int a) {

        Vec3 position = Minecraft.getInstance().gameRenderer.getMainCamera().getPosition();
        poseStack.translate(v.x - position.x, v.y - position.y, v.z - position.z);


    }

    public static void levelPoseStack(PoseStack poseStack, Vector3f v) {
    ...
    public static void levelPoseStack(PoseStack poseStack, Entity e) {

        Vec3 position = Minecraft.getInstance().gameRenderer.getMainCamera().getPosition();
        double x = Mth.lerp(Minecraft.getInstance().getPartialTick(), e.xOld, e.getX());
        double y = Mth.lerp(Minecraft.getInstance().getPartialTick(), e.yOld, e.getY());
        double z = Mth.lerp(Minecraft.getInstance().getPartialTick(), e.zOld, e.getZ());
    ...
    public void fill(int pMinX, int pMinY, int pMaxX, int pMaxY, int pColor,int  aa) {
...
//
    ...
    public void fill(int pMinX, int pMinY, int pMaxX, int pMaxY, int pColor){
       ...
       guiGraphics.fill( pMinX,  pMinY,  pMaxX,  pMaxY,  pColor);

    }
    public void fill(Vector3d[] pos, float[] color){

        fill(poseStack(),pos,color);
    ...
    public void fill(PoseStack poseStack, Vector3d[] pos, float[] color) {
        ...
        vertexConsumer= bufferSource.getBuffer(
                ShaderManager.getRenderType(
                        new ResourceLocation(IPC.MODID,"rendertype_lines"),
                        DefaultVertexFormat.NEW_ENTITY,
                        VertexFormat.Mode.QUADS).apply(EntityBar.EntityBar_TEXTURE));

        vertexConsumer = bufferSource.getBuffer(RenderTypes.TranslucentEntity(EntityBar.EntityBar_TEXTURE));
        ...
        Matrix3f normalMatrix = pose.normal();    // 顶点法线矩阵（光照计算）
...
//顺时针
    ...
    private void addVertex(VertexConsumer consumer, Matrix4f worldMatrix, Matrix3f normalMatrix,
        ...
        consumer.vertex(worldMatrix,
                        (float) pos.x, (float) pos.y, (float) pos.z)//空间坐标
                .color(color[0],
                       color[1],
                       color[2],
                       color[3]) // RGBA颜色
...
//
    ...
    public void renderLaser(Vector3f poss1, Vector3f poss2, float r, float slanting) {

        renderLaser(new float[]{poss1.x, poss1.y, poss1.z}, new float[]{poss2.x, poss2.y, poss2.z}, r, slanting);
    ...
    public void renderLaser(float[] poss1, float[] poss2, float r) {
        ...
        renderLaser(new Vector3f(poss1[0], poss1[1], poss1[2]), new Vector3f(poss2[0], poss2[1], poss2[2]), r);

    }


    public void renderLaser(Vector3f poss1, Vector3f poss2, float r) {

        if(poss1.y<poss2.y){//因为第二点高于第一点 无法旋转  未知原因
        ...
        Vector3f v1 = minecraft.gameRenderer.getMainCamera().getPosition().toVector3f();
        Vector3f p1 = new Vector3f();
        poss1.sub(v1, p1);
        Vector3f p2 = new Vector3f();
        poss2.sub(v1, p2);
        Vector3f v2 = Line.crossProduct(p1, p2);//横向180度法向量
        Vector3f p3 = new Vector3f();
        poss1.sub(v2, p3);
        Vector3f p4 = new Vector3f();
        poss2.sub(v2, p4);
        Vector3f v4 = Line.crossProduct(p3, p4);//竖向180度法向量

        float dx = poss1.x - poss2.x();
        float dy = poss1.y - poss2.y();
        float dz = poss1.z - poss2.z();

        float y2 = (float) Math.sqrt(dx * dx + dy * dy + dz * dz);//poss1.distance(poss2);

        float d = y2;
        float yy = dy;

        float y1 = (float) Math.pow(Math.max(0.000001, (Math.pow(d * d / yy, 2)) - 2 * d * d + yy * yy), 0.5);
        Vector3f v3 = new Vector3f(//默认角度线朝向
                (float) (- ( poss1.x - poss2.x)),
                dy>0?(y1):-y1,
                (float) (-((poss1.z - poss2.z))));

     //  DeBug.tell(v3.x+"  "+v3.y+"  "+v3.z);
     //  DeBug.tell(v4.x+"  "+v4.y+"  "+v4.z);
      //  getPoseStack().pushPose();
      //  Render.levelPoseStack(getPoseStack(),  poss1.x - poss2.x,
      //          y1,
      //          poss1.z - poss2.z);
      //   Render.bindCamera(getPoseStack());
      //  guiGraphics.blit(texture, 0, 0, 0, 0, (int) u1, (int) u2);
      //  getPoseStack().popPose();

        float angle1 = (float) (Math.PI * ((Line.dot(v4, v3)) / 180));

        float angle2 = (float) (Math.PI * ((Line.dot(v2, v3)) / 180));
       // System.out.println(angle1);
        if (angle2 < Math.PI * 0.5) {
            angle1 *= -1;
        }
        renderLaser(new float[]{poss1.x, poss1.y, poss1.z}, new float[]{poss2.x, poss2.y, poss2.z}, r, angle1);
    }
    ...
    public void renderLaserss(float[] poss1, float[] poss2, float r) {
