...
//注意  实体注册需写在这之间------------------------------
    ...
    public static <T extends Entity> RegistryObject<EntityType<T>> registerEffect(
            String name, EntityType.EntityFactory<T> entity,
            float width, float height, int primaryEggColor, int secondaryEggColor) {

    //    System.out.println(456);

        return ENTITIES.register(name,
                () -> EntityType.Builder.of(entity, MobCategory.CREATURE).sized(width, height).noSave().fireImmune().build(new ResourceLocation(IPC.MODID, name).toString()));
    }
